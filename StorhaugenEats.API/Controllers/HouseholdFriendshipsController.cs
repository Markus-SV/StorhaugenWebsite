...
public class HouseholdFriendshipsController : ControllerBase
{
    ...
    [HttpPost("request")]
    public async Task<ActionResult<HouseholdFriendshipDto>> RequestFriendship([FromBody] SendFriendRequestDto dto)
    {
        var userId = await _currentUserService.GetOrCreateUserIdAsync();
        var user = await _context.Users.FindAsync(userId);
        if (user?.CurrentHouseholdId == null)
    ...
    [HttpPost("{id}/respond")]
    public async Task<ActionResult<HouseholdFriendshipDto>> Respond(Guid id, [FromBody] RespondFriendRequestDto dto)
    {
        var userId = await _currentUserService.GetOrCreateUserIdAsync();
        var user = await _context.Users.FindAsync(userId);
        if (user?.CurrentHouseholdId == null)
